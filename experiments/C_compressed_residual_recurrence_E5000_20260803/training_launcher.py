#!/usr/bin/env python3
"""Launch one paired compressed residual-recurrence training run."""
...
protocol = {
    "scientific_question": (
        "does_residual_solution_recurrence_survive_genuine_rank_compression"
    ),
    "arm": f"residual_rank{RANK}",
    ...
    "walkers": 1000,
    "centered_current_batch_max_rank": 999,
    "rank": RANK,
    "genuine_current_batch_compression": RANK < 999,
...
def _append_monitor(epoch, variance, direction, prior, correction, active_rank):
    ...
    with path.open("a", newline="") as handle:
        ...
        writer.writerow(
            [epoch, variance, direction, prior, correction, active_rank, time.time()]
        )
...
def monitored_append(logdir, epoch, metrics):
    ...
    _append_monitor(
        epoch_out, variance, direction, prior, correction, active_rank
    )

    reason = None
    values = (energy, variance, direction, prior, correction, active_rank)
