#!/bin/bash
#SBATCH --job-name=C-hist-compress
...
ROOT=${ROOT:-/scratch/dexuan1/runs/C_wssr_rank1600_history_compression_audit_20260802}
...
cp experiments/C_rank1600_history_compression_audit/README.md \
  "${OUT}/preregistered_protocol.md"

python experiments/C_rank1600_history_compression_audit/audit.py \
  --run "${SOURCE_RUN}" \
  --epoch "${SOURCE_EPOCH}" \
