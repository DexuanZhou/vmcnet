#!/bin/bash
#SBATCH --job-name=N2-adcomp-E500
...
case $METHOD in
  0) VAR=A_rank400_hard; RANK=400; MODE=none; BETA=0; COMP=0;;
  1) VAR=B_rank400_beta01; RANK=400; MODE=adaptive_complement; BETA=.1; COMP=.0001;;
  2) VAR=C_rank400_beta02; RANK=400; MODE=adaptive_complement; BETA=.2; COMP=.0001;;
  3) VAR=D_rank800_hard; RANK=800; MODE=none; BETA=0; COMP=0;;
  *) exit 2;;
esac
RUN=/scratch/dexuan1/runs/N2_adaptive_complement_E500/$VAR/seed$SEED
META=${RUN}.metadata
[[ -f $SOURCE/checkpoints/5000.npz && ! -e $RUN && ! -e $META ]] || exit 2
...
export WSSR_STAGE3_SEED=$SEED SMOKE_TIMING_DIR=$META
...
printf 'variant=%s\nseed=%s\nrank=%s\nbeta=%s\nsource_checkpoint=%s\n' "$VAR" "$SEED" "$RANK" "$BETA" "$SOURCE/checkpoints/5000.npz" > "$META/protocol.txt"
echo 'timestamp_unix,memory_used_mib' > "$META/gpu_memory_poll.csv"
(while true; do T=$(date +%s.%N); nvidia-smi --query-gpu=memory.used --format=csv,noheader,nounits | awk -v t="$T" '{print t "," $1}' >> "$META/gpu_memory_poll.csv" || true; sleep 1; done) & MON=$!
