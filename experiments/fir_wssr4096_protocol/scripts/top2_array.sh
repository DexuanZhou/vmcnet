...
case "${SLURM_ARRAY_TASK_ID}" in
  0) SYSTEM=C;  NAME=C_rank800_eta02_lr0002; ETA=0.2; LR=0.002;;
  1) SYSTEM=C;  NAME=C_rank800_eta08_lr02;   ETA=0.8; LR=0.02;;
  2) SYSTEM=N2; NAME=N2_rank800_eta03_lr0002; ETA=0.3; LR=0.002;;
  3) SYSTEM=N2; NAME=N2_rank800_eta02_lr0002; ETA=0.2; LR=0.002;;
  *) exit 2;;
esac
RANK=800; WARM=2
if [[ ${SYSTEM} == C ]]; then
...
ROOT=/scratch/dexuan1/runs/wssr4096_top2/${SYSTEM}
RUN=${ROOT}/${NAME}
META=${ROOT}/metadata/${NAME}
[[ -f ${SOURCE}/checkpoints/5000.npz ]] || { echo "missing ${SOURCE}/checkpoints/5000.npz" >&2; exit 2; }
[[ ! -e ${RUN} && ! -e ${META} ]] || { echo "refusing overwrite: ${RUN} or ${META}" >&2; exit 2; }
module --force purge
module load StdEnv/2023 gcc/12.3 python/3.11.5
source /home/dexuan1/projects/rrg-ortner/dexuan1/venvs/vmcnet311/bin/activate
export WANDB_MODE=disabled WANDB_DISABLED=true WANDB_SILENT=true
export XLA_PYTHON_CLIENT_PREALLOCATE=false VMCNET_PROFILE_TIMING=1 SMOKE_TIMING_DIR=${META}
...
GPUMON=$!
cleanup(){ kill ${GPUMON} 2>/dev/null || true; wait ${GPUMON} 2>/dev/null || true; }
...
python experiments/fir_n4096_smoke/scripts/timed_reload_launcher.py \
 ...
 --config.vmc.nchains=4096 --config.eval.nchains=4096 --config.vmc.nepochs=2000 --config.vmc.nburn=5000 \
 --config.vmc.nsteps_per_param_update=10 --config.vmc.check_for_nans=True \
 --config.vmc.disable_checkpointing=False --config.vmc.checkpoint_every=500 --config.vmc.best_checkpoint_every=500 \
...
PID=$!
python experiments/fir_wssr4096_protocol/monitor_screen.py --pid ${PID} --run ${RUN} --out ${META}/early_stop.json &
SMON=$!
wait ${PID}; STATUS=$?
wait ${SMON} 2>/dev/null || true
set -e
cleanup
trap - EXIT INT TERM
exit ${STATUS}
