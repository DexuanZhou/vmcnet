#!/bin/bash
#SBATCH --job-name=C-gtransport-E5k
...
ROOT=/scratch/dexuan1/runs/C_wssr_gradient_transport_etaG02_20260803
case "${SLURM_ARRAY_TASK_ID}" in
  0) ARM=expG02_vanilla;     SEED=0; TRANSPORT=False ;;
  1) ARM=expG02_transported; SEED=0; TRANSPORT=True  ;;
  2) ARM=expG02_vanilla;     SEED=1; TRANSPORT=False ;;
  3) ARM=expG02_transported; SEED=1; TRANSPORT=True  ;;
...
python experiments/C_rank1600_eta_memory_E5000_20260802/training_launcher.py \
  ...
  --config.vmc.optimizer.wssr_warm_svd_right.adaptive_g_average=True \
  --config.vmc.optimizer.wssr_warm_svd_right.eta_g_schedule=exponential_growth \
  --config.vmc.optimizer.wssr_warm_svd_right.eta_g_max=0.2 \
  --config.vmc.optimizer.wssr_warm_svd_right.eta_g_tau=1000.0 \
  --config.vmc.optimizer.wssr_warm_svd_right.eta_bias_correction=False \
  --config.vmc.optimizer.wssr_warm_svd_right.enable_gradient_transport="${TRANSPORT}" \
