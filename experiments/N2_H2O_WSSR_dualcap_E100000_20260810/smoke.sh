...
case "${SYSTEM}" in
  N2)
    ...
    RUN=/scratch/dexuan1/runs/N2_R2016_WSSR_rank1600_dualcap_smoke_20260810
  ...
  H2O)
    ...
    CHECKPOINT=${SOURCE}/checkpoints/5000.npz
    RUN=/scratch/dexuan1/runs/H2O_WSSR_rank1600_dualcap_smoke_20260810
...
purpose=metric-matched dual-cap tail WSSR rank1600 smoke gate
system=${SYSTEM}
...
checkpoint_epoch=20
checkpoint_reload_validation=True
EOF
...
vmc-molecule \
  ...
  --config.vmc.checkpoint_every=20 \
  --config.vmc.best_checkpoint_every=20 \
...
RUN="${RUN}" META="${META}" PYTHONPATH="${SNAPSHOT}" python - <<'PY'
import os
import numpy as np
import jax
...
energy = np.atleast_1d(np.loadtxt(os.path.join(run, "energy.txt")))
variance = np.atleast_1d(np.loadtxt(os.path.join(run, "variance.txt")))
assert energy.size == 20, energy.size
assert variance.size == 20, variance.size
assert np.all(np.isfinite(energy))
assert np.all(np.isfinite(variance))
checkpoint = os.path.join(run, "checkpoints", "20.npz")
with np.load(checkpoint, allow_pickle=True) as z:
    assert int(z["e"]) == 19
    assert z["o_format"].item() == "pytree_leaves_v1"
    nleaves = int(z["o_num_leaves"].item())
    assert nleaves > 0
    assert f"o_leaf_{nleaves - 1:06d}" in z.files

# Exercise the production reload path, not just the ZIP central directory.
state = io.reload_vmc_state(
    os.path.dirname(checkpoint), os.path.basename(checkpoint)
)
optimizer_leaves = jax.tree_util.tree_leaves(state[3])
largest_optimizer_leaf = max(np.asarray(x).nbytes for x in optimizer_leaves)
assert largest_optimizer_leaf > 4 * 1024**3
memory = np.genfromtxt(
    os.path.join(meta, "gpu_memory_mib.csv"), delimiter=",", dtype=str
)
used = np.asarray([float(row[2]) for row in np.atleast_2d(memory)])
with open(os.path.join(meta, "validation.txt"), "w") as f:
    f.write("finite_steps=20\n")
    f.write(f"peak_gpu_memory_mib={np.nanmax(used):.0f}\n")
    f.write("checkpoint_count=2\n")
    f.write("checkpoint_format=pytree_leaves_v1\n")
    f.write(f"optimizer_leaves={nleaves}\n")
    f.write(f"largest_optimizer_leaf_bytes={largest_optimizer_leaf}\n")
    f.write(f"regular_checkpoint_bytes={os.path.getsize(checkpoint)}\n")
