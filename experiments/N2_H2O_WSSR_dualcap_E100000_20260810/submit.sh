...
TAG=dualcap_etaS03_etaG03
[[ ! -e "${OUT}" ]] || { echo "refusing to overwrite ${OUT}" >&2; exit 2; }

common_export="ALL,ETA_S=0.3,ETA_G=0.3,RUN_TAG=${TAG}"

n2_smoke=$(sbatch --parsable --job-name=N2-WSSR-r1600-dual-smoke \
  --export="${common_export},SYSTEM=N2" "${HERE}/smoke.sh")
n2_stage1=$(sbatch --parsable --job-name=N2-WSSR-r1600-dual-50k \
  --export="${common_export},SYSTEM=N2" --dependency=afterok:${n2_smoke} \
  "${HERE}/stage1_train.sh")
n2_stage2=$(sbatch --parsable --job-name=N2-WSSR-r1600-dual-100k \
  --export="${common_export},SYSTEM=N2" --dependency=afterok:${n2_stage1} \
  "${HERE}/stage2_resume_eval.sh")

h2o_smoke=$(sbatch --parsable --job-name=H2O-WSSR-r1600-dual-smoke \
  --export="${common_export},SYSTEM=H2O" "${HERE}/smoke.sh")
h2o_stage1=$(sbatch --parsable --job-name=H2O-WSSR-r1600-dual-50k \
  --export="${common_export},SYSTEM=H2O" --dependency=afterok:${h2o_smoke} \
  "${HERE}/stage1_train.sh")
h2o_stage2=$(sbatch --parsable --job-name=H2O-WSSR-r1600-dual-100k \
