#!/bin/bash
# Five-level WSSR pilot with matched 2,000 total epochs and 1,000 L2 epochs.
#SBATCH --job-name=C-WSSR-5level
...
INITIAL=${ROOT}/optimizer_initial/wssr_warm_svd_right
ARM=${ROOT}/pilot/wssr_warm_svd_right/five_level
...
RUN_L0=${ARM}/train_L0_to250
CONFIG_L05=${ARM}/config_L05
CONVERTED_L05=${ARM}/converted_L05
RUN_L05=${ARM}/train_L05_to500
CONFIG_L1=${ARM}/config_L1
CONVERTED_L1=${ARM}/converted_L1
RUN_L1=${ARM}/train_L1_to750
CONFIG_L15=${ARM}/config_L15
CONVERTED_L15=${ARM}/converted_L15
RUN_L15=${ARM}/train_L15_to1000
CONFIG_L2=${ARM}/config_L2
CONVERTED_L2=${ARM}/converted_L2
RUN_L2=${ARM}/train_L2_to2000

continue_level "${INITIAL}" checkpoints/1.npz "${RUN_L0}" 250

make_level_config "${RUN_L0}/config.json" L05 "${CONFIG_L05}"
convert_level "${RUN_L0}/config.json" "${CONFIG_L05}/config.json" \
  "${RUN_L0}/checkpoints/250.npz" "${CONVERTED_L05}"
continue_level "${CONVERTED_L05}" multilevel.npz "${RUN_L05}" 500

make_level_config "${RUN_L05}/config.json" L1 "${CONFIG_L1}"
convert_level "${RUN_L05}/config.json" "${CONFIG_L1}/config.json" \
  "${RUN_L05}/checkpoints/500.npz" "${CONVERTED_L1}"
continue_level "${CONVERTED_L1}" multilevel.npz "${RUN_L1}" 750

make_level_config "${RUN_L1}/config.json" L15 "${CONFIG_L15}"
convert_level "${RUN_L1}/config.json" "${CONFIG_L15}/config.json" \
  "${RUN_L1}/checkpoints/750.npz" "${CONVERTED_L15}"
continue_level "${CONVERTED_L15}" multilevel.npz "${RUN_L15}" 1000

make_level_config "${RUN_L15}/config.json" L2 "${CONFIG_L2}"
convert_level "${RUN_L15}/config.json" "${CONFIG_L2}/config.json" \
  "${RUN_L15}/checkpoints/1000.npz" "${CONVERTED_L2}"
