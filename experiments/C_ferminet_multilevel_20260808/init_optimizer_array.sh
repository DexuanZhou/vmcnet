...
SOURCE=${ROOT}/kfac_l0_pre1000
VENV=/home/dexuan1/projects/rrg-ortner/dexuan1/venvs/vmcnet311

case "${SLURM_ARRAY_TASK_ID}" in
  0) METHOD=spring ;;
  1) METHOD=wssr_warm_svd_right ;;
  *) exit 2 ;;
esac
RUN=${ROOT}/optimizer_initial/${METHOD}
...
COMMON=(
  ...
  --config.vmc.best_checkpoint_every=1
...
if [[ "${METHOD}" == spring ]]; then
  ...
  vmc-molecule "${COMMON[@]}" \
    ...
    --config.vmc.optimizer.wssr_warm_svd_right.eta_S=0.3 \
