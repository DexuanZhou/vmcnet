#!/bin/bash
#SBATCH --job-name=N2-spring4096-mu
...
set -euo pipefail
MUS=(0.90 0.95); LABELS=(mu090 mu095)
MU=${MU_OVERRIDE:-${MUS[${SLURM_ARRAY_TASK_ID}]}}; LABEL=${LABEL_OVERRIDE:-${LABELS[${SLURM_ARRAY_TASK_ID}]}}
REPO=/scratch/dexuan1/vmcnet; PKG=${REPO}/experiments/fir_n4096_smoke
ROOT=/scratch/dexuan1/runs/fir_n4096_corrected_retry1/N2; SOURCE=${ROOT}/kfac_preliminary
NAME=N2eq_R2068_spring_lr0005_${LABEL}_n4096_e200
...
find "${RUN}" -name '*.npz' -print > "${META}/npz_files.txt" 2>/dev/null || true; exit "${STATUS}"
