#!/bin/bash
#SBATCH --job-name=N2-spring4096-stability
...
LABELS=(lr0005 lr0002 lr0001)
LR=${LRS[${SLURM_ARRAY_TASK_ID}]}; LABEL=${LABELS[${SLURM_ARRAY_TASK_ID}]}
...
NAME=N2eq_R2068_spring_${LABEL}_n4096_e200
RUN=${ROOT}/spring_stability/${NAME}
...
python - <<'PY' > "${META}/versions.txt"
import platform,jax,jaxlib
print('Python',platform.python_version()); print('JAX',jax.__version__); print('jaxlib',jaxlib.__version__); print(jax.devices())
PY
...
python "${PKG}/scripts/timed_reload_launcher.py" \
 ...
 --config.vmc.nchains=4096 --config.vmc.nburn=5000 --config.vmc.nepochs=200 --config.vmc.nsteps_per_param_update=10 \
 ...
 --config.vmc.optimizer_type=spring --config.vmc.optimizer.spring.learning_rate="${LR}" \
 --config.vmc.optimizer.spring.mu=0.99 --config.vmc.optimizer.spring.damping=0.001 \
 --config.vmc.optimizer.spring.constrain_norm=True --config.vmc.optimizer.spring.norm_constraint=0.001 \
 --config.vmc.optimizer.spring.schedule_type=inverse_time --config.vmc.optimizer.spring.learning_decay_rate=0.0001
STATUS=$?; set -e; cleanup; trap - EXIT INT TERM
find "${RUN}" -name '*.npz' -print > "${META}/npz_files.txt" 2>/dev/null || true
exit "${STATUS}"
