#!/bin/bash
#SBATCH --job-name=C-r1600-comp-ema
...
case "${SLURM_ARRAY_TASK_ID}" in
  0) RHO=0.9; LABEL=rho09 ;;
  1) RHO=0.99; LABEL=rho099 ;;
  *) exit 2 ;;
esac

SOURCE=/scratch/dexuan1/runs/C_wssr_rank1600_eta03_lr004_E100000_resume1
CHECKPOINT=${SOURCE}/checkpoints/100000.npz
RUN=/scratch/dexuan1/runs/C_wssr_rank1600_beta0001_compema_${LABEL}_E5000
META=${RUN}_metadata
...
complement_state_definition=c_k=rho*c_prev+(1-rho)*instantaneous_beta_capped_complement
complement_state_decay=${RHO}
complement_state_relative_cap=0.01
...
vmc-molecule \
  ...
  --config.vmc.optimizer.wssr_warm_svd_right.complement_state_decay="${RHO}" \
...
for epoch in 101000 102000 103000 104000 105000; do
  [[ -f "${RUN}/checkpoints/${epoch}.npz" ]] || exit 4
done
