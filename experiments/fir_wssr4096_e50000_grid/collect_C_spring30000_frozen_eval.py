...
RUN=pathlib.Path('/scratch/dexuan1/runs/C_spring_mu099_epoch30000_frozen_eval_burn100k_n8192')
OUT=pathlib.Path(__file__).parent/'results/C_spring_mu099_epoch30000_frozen_eval_burn100k_n8192.json'
...
center=np.nanmean(x,axis=1,keepdims=True);tv=np.nanmean(np.abs(x-center),axis=1,keepdims=True);clipped=np.clip(x,center-5*tv,center+5*tv);accept=np.loadtxt(RUN/'eval/accept_ratio.txt')
result={'source_checkpoint':'/scratch/dexuan1/runs/C_spring_mu099_after_kfac5000_E50000/checkpoints/30000.npz','checkpoint_training_epoch':30000,'reference_energy':-37.84471,'shape':list(x.shape),'nonfinite_samples':int((~finite).sum()),'acceptance_mean':float(np.mean(accept)),'unclipped':stats(x),'training_style_clipped':stats(clipped)}
