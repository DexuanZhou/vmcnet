#!/usr/bin/env python3
"""Collect paired E5000 frozen results without submitting follow-up work."""
...
ROOT = Path("/scratch/dexuan1/runs/C_current_subspace_spectral_history_E5000_20260812")
OUT = Path(__file__).resolve().parent
ARMS = ("current_only", "full_matrix_ema", "uniform_spectrum", "cluster_adaptive")
rows = []
for arm in ARMS:
    for seed in (0, 1):
        ...
        rows.append(
            {
                ...
                "eta_mean_final100": float(
                    np.nanmedian([float(row["eta_mean"]) for row in monitor[-100:]])
                ),
                "eta_head_final100": float(
                    np.nanmedian([float(row["eta_head"]) for row in monitor[-100:]])
                ),
                "eta_tail_final100": float(
                    np.nanmedian([float(row["eta_tail"]) for row in monitor[-100:]])
                ),
            }
        )
...
for row in rows:
    print(
        ...
        f"{row['eta_head_final100']:.3f}/{row['eta_mean_final100']:.3f}/"
        f"{row['eta_tail_final100']:.3f} |"
    )
