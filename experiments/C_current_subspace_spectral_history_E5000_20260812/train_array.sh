#!/bin/bash
#SBATCH --job-name=C-spec-history
...
ROOT=/scratch/dexuan1/runs/C_current_subspace_spectral_history_E5000_20260812
SEED=$((SLURM_ARRAY_TASK_ID / 4))
case "$((SLURM_ARRAY_TASK_ID % 4))" in
  0) ARM=current_only;       ETA_S=0.0;  MODE=none ;;
  1) ARM=full_matrix_ema;    ETA_S=0.3;  MODE=none ;;
  2) ARM=uniform_spectrum;   ETA_S=0.3;  MODE=uniform_spectrum ;;
  3) ARM=cluster_adaptive;   ETA_S=0.95; MODE=cluster_adaptive ;;
  *) exit 2 ;;
esac
...
export WSSR_REDUCED_METRIC_HISTORY_MODE="${MODE}"
...
python experiments/C_current_subspace_spectral_history_E5000_20260812/training_launcher.py \
  ...
  --config.vmc.optimizer.wssr_warm_svd_right.reduced_metric_history_mode="${MODE}" \
  --config.vmc.optimizer.wssr_warm_svd_right.spectral_history_cluster_gap=0.01 \
  --config.vmc.optimizer.wssr_warm_svd_right.spectral_history_noise_scale=1.0 \
  --config.vmc.optimizer.wssr_warm_svd_right.spectral_history_drift_scale=1.0 \
