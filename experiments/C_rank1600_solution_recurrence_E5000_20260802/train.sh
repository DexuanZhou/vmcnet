#!/bin/bash
#SBATCH --job-name=C-r1600-solrec
...
ROOT=/scratch/dexuan1/runs/C_rank1600_solution_recurrence_E5000_20260802
SEED=${WSSR_PAIRED_SEED:-0}
UPDATES=${WSSR_UPDATES:-5000}
STAGE=${WSSR_STAGE:-train}
case "${SLURM_ARRAY_TASK_ID}" in
  0) ARM=naive_mu099; MODE=naive ;;
  1) ARM=residual_mu099; MODE=residual ;;
  *) echo "unexpected task ${SLURM_ARRAY_TASK_ID}" >&2; exit 2 ;;
esac

RUN=${ROOT}/${STAGE}/${ARM}/seed${SEED}
MONITOR=${ROOT}/metadata_${STAGE}/${ARM}/seed${SEED}
...
export WSSR_ARM="${ARM}"
export WSSR_RECURRENCE_MODE="${MODE}"
export WSSR_UPDATES="${UPDATES}"

mkdir -p "$(dirname "${RUN}")" /scratch/dexuan1/runs/logs
cd /scratch/dexuan1/vmcnet
python experiments/C_rank1600_solution_recurrence_E5000_20260802/training_launcher.py \
  ...
  --config.vmc.optimizer.wssr_warm_svd_right.solution_recurrence_mode="${MODE}" \
