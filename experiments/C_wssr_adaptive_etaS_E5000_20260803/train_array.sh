#!/bin/bash
#SBATCH --job-name=C-adapt-etaS-E5k
...
ROOT=/scratch/dexuan1/runs/C_wssr_adaptive_etaS_E5000_20260803

case "${SLURM_ARRAY_TASK_ID}" in
  0)
    ARM=linear_T1000
    ETA_S_SCHEDULE=linear_warmup
    ETA_S_WARMUP_STEPS=1000
    ETA_S_TAU=1000.0
    ;;
  1)
    ARM=exponential_tau1000
...
python experiments/C_rank1600_eta_memory_E5000_20260802/training_launcher.py \
  ...
  --config.vmc.optimizer.wssr_warm_svd_right.eta_S_schedule="${ETA_S_SCHEDULE}" \
  --config.vmc.optimizer.wssr_warm_svd_right.eta_S_max=0.95 \
  --config.vmc.optimizer.wssr_warm_svd_right.eta_S_warmup_steps="${ETA_S_WARMUP_STEPS}" \
  --config.vmc.optimizer.wssr_warm_svd_right.eta_S_tau="${ETA_S_TAU}" \
