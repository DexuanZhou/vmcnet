#!/bin/bash
#SBATCH --job-name=N2-r200-snr-e1000
...
RUN=${ROOT}/train_e1000
META=${ROOT}/metadata/train_e1000
VENV=/home/dexuan1/projects/rrg-ortner/dexuan1/venvs/vmcnet311
[[ -f "${ROOT}/health_summary.json" ]] || exit 2
python -c 'import json,sys; sys.exit(0 if json.load(open(sys.argv[1]))["healthy"] else 2)' \
  "${ROOT}/health_summary.json"
...
vmc-molecule \
  ...
  --config.vmc.optimizer.wssr_warm_svd_right.experimental_mode=cluster_envelope_ritz_snr \
  ...
  --config.vmc.optimizer.wssr_warm_svd_right.cluster_snr_gap_threshold=0.05 \
