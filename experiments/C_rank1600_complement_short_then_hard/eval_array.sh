#!/bin/bash
#SBATCH --job-name=C-short-hard-eval
...
if (( SLURM_ARRAY_TASK_ID < 5 )); then
  LABEL=on1000; CUTOFF=101000; I=${SLURM_ARRAY_TASK_ID}
else
  LABEL=on2000; CUTOFF=102000; I=$((SLURM_ARRAY_TASK_ID - 5))
fi
EPOCH=$((101000 + 1000 * I))
...
if (( EPOCH <= CUTOFF )); then SOURCE=${EMA}; else SOURCE=${HARD}; fi
RUN=/scratch/dexuan1/runs/C_wssr_rank1600_beta0001_short_then_hard_frozen/${LABEL}_epoch${EPOCH}
