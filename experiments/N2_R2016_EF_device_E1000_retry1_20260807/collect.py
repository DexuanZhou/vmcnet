...
ROOT = Path("/scratch/dexuan1/runs/N2_R2016_EF_device_E1000_retry1_20260807")
HERE = Path("/scratch/dexuan1/vmcnet/experiments/N2_R2016_EF_device_E1000_retry1_20260807")
...
def frozen_statistics_path(arm, step):
    """Return the endpoint statistics, including legacy auto-suffixed runs."""
    base = ROOT / "frozen" / f"{arm}_e{step}"
    candidates = (
        base,
        Path(f"{base}_1"),
        Path(f"{base}_retry"),
        Path(f"{base}_retry_1"),
        Path(f"{base}_retry2"),
        Path(f"{base}_retry2_1"),
    )
    for candidate in candidates:
        path = candidate / "eval" / "statistics.json"
        if path.exists():
            return path
    return base / "eval" / "statistics.json"
...
for arm in ARMS:
    ...
    for step in STEPS:
        stats_path = frozen_statistics_path(arm, step)
