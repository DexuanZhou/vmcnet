...
def test_parse_config_with_valid_flags_including_tuple(mocker):
    ...
    mocker.patch(
        ...
        [
            "vmcnet",
            "--config.model.type=explicit_antisym",
    ...
    expected_config = get_default_config_with_chosen_model("explicit_antisym")
...
def test_parse_config_accepts_wssr_warm_svd_optimizer_type(mocker):
    """Test that wssr_warm_svd is accepted as a visible optimizer option."""
    flag_values = flags.FlagValues()
    mocker.patch(
        "sys.argv",
        ["vmcnet", "--config.vmc.optimizer_type=wssr_warm_svd"],
    )
    expected_config = get_default_config_with_chosen_model("ferminet")
    expected_config.vmc.optimizer_type = "wssr_warm_svd"
