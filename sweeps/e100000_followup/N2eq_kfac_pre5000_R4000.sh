#!/bin/bash
#SBATCH --job-name=N2eq-kfac-pre5000-r4000
...
RUN_DIR=/scratch/dexuan1/runs/kfac_pre/N2eq_R4000_kfac_pre5000
...
{"system":"N2eq","bond_length_bohr":4.0,"run_name":"N2eq_R4000_kfac_pre5000","family":"kfac_pre","nepochs":5000,"nchains":1000,"nburn":5000,"nsteps_per_param_update":10,"clip_center":"mean","clip_threshold":5.0,"eval_nepochs":0,"optimizer_type":"kfac","learning_rate":0.05,"schedule_type":"inverse_time","learning_decay_rate":0.0001,"checkpoint_every":5000,"ion_pos":"((0.0,0.0,-2.0),(0.0,0.0,2.0))","ion_charges":"(7.0,7.0)","nelec":"(7,7)"}
EOF
...
vmc-molecule --config.logdir="${RUN_DIR}" --config.save_to_current_datetime_subfolder=False --config.subfolder_name=NONE \
 --config.problem.ion_pos='((0.0,0.0,-2.0),(0.0,0.0,2.0))' --config.problem.ion_charges='(7.0,7.0)' --config.problem.nelec='(7,7)' \
