#!/bin/bash
#SBATCH --job-name=N2eq-kfac-pre5000-r2016
...
RUN_DIR=/scratch/dexuan1/runs/e100000_followup/N2eq/N2eq_kfac_pre5000
...
vmc-molecule --config.logdir="${RUN_DIR}" --config.save_to_current_datetime_subfolder=False --config.subfolder_name=NONE \
 --config.problem.ion_pos='((0.0,0.0,-1.008),(0.0,0.0,1.008))' --config.problem.ion_charges='(7.0,7.0)' --config.problem.nelec='(7,7)' \
